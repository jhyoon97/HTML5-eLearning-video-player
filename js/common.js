...
var prevPageURL =
  "./" +
  pagingURL(htmlChasi, false, true) +
  "_" +
  pagingURL(htmlPage, false, false) +
  ".html"; // 형태 :: ./00_00.html, 이전 페이지의 파일명
var nextPageURL =
  "./" +
  pagingURL(htmlChasi, true, true) +
  "_" +
  pagingURL(htmlPage, true, false) +
  ".html"; // 형태 :: ./00_00.html, 다음 페이지의 파일명
var intervalMs = 30; // 인터벌의 실행 간격
var volSave; // 음소거 이전의 볼륨값 저장
var con_w = 1000; // 콘텐츠의 기준 가로 사이즈 (scale 조절에 쓰임)
var con_h = 620; // 콘텐츠의 기준 세로 사이즈 (scale 조절에 쓰임)
...
window.onload = function () {
  if (!$("#mainMedia").attr("src")) {
    if ($("#mainMedia").is("video")) {
      mediaURL =
        "./vod/" +
        replaceZero(htmlChasi) +
        "_" +
        replaceZero(htmlPage) +
        ".mp4";
...
function loadComplete() {
  ...
  if (isMobile) {
    // 모바일일 때 자동재생이 안되므로
    $("#content").append(
      '<div id="mobile_play"><img src="./images/mobile_play.png"/></div>'
    ); // 재생유도버튼 추가
...
function setIndexURL() {
  ...
  for (var i = 1; i <= total_id; i++) {
    ...
    var location_html =
      replaceZero(htmlChasi) + "_" + replaceZero(move_value[i - 1]) + ".html";
  ...
  var markInterval = setInterval(function () {
    for (var i = 0; i < markArr.length; i++) {
      ...
      $("#quickMark").append(
        '<div class="mark mark' +
          (i + 1) +
          '"><div class="displayMark"></div><div class="text">' +
          markArr[i].text +
          "</div></div>"
      );
      $tmp.append(
        '<div class="text text' + i + '">' + markArr[i].text + "</div>"
      );
      $target = $(".mark" + (i + 1));
      ...
      $target
        ...
        .css(
          "margin-left",
          -(
            ($target.find(".text").width() +
              px_to_num($target.find(".text").css("padding-left")) * 2) /
              2 -
            $target.width() / 2
          )
        );
      $target.css(
        "left",
        (markArr[i].time / media.duration) * $bar_event.width()
      );
      (function (currentI) {
...
function ansChk(qNum) {
  ...
  if (ans_num[qNum - 1] == clickNum) {
    ...
    $(".q" + qNum + " .display_correct").css(
      "background",
      "url(./images/q_o.png)"
    );
    $(".q" + qNum + " .display_correct").css("visibility", "visible");
    $(".q" + qNum + " .viewCon").show();
    $("#review" + qNum + " .re_correct").css(
      "background",
      "url(./images/q_rev_o.png)"
    );
    return;
  } else {
    if (change_reTest == 0) {
...
function call_alert(str) {
  ...
  if (str === "correct") {
    $("#alert_wrap").append(
      '<div id="alert_correct" class="alert_content"></div>'
    );
  } else if (str === "wrong") {
    $("#alert_wrap").append(
      '<div id="alert_wrong" class="alert_content"></div>'
    );
  } else if (str === "re") {
...
function call_imagePopup(url) {
  ...
  urlArray = url.split(",");
  $("#content").append(
    '<div id="popup_wrap"><div id="popup_close"></div></div>'
  );
